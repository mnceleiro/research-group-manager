...
case class CongressVO(id: Long, title: String, name: String, place: String, country: String, start: String, end: String, international: Boolean, authors: Seq[AuthorOfCongressVO])
...
object CongressVO {
  ...
  def toVO(c: Congress): CongressVO = {
    return CongressVO(c.id, c.title, c.name, c.place, c.country, c.start, c.end, c.international, Seq())
  }
  ...
  def toCongressVOWithAuthors(data: Seq[(Congress, Option[AuthorCongress], Option[Author])]): Option[CongressVO] = {
    ...
    if (data.length == 0) return None
      ...
      val p = data.head._1
      Some(CongressVO(p.id, p.title, p.name, p.place, p.country, p.start, p.end, p.international, Seq()))
      
    } else {
      Some(CongressVO(p.id, p.title, p.name, p.place, p.country, p.start, p.end, p.international, data.map(t => {
        val mid = t._2.get
        val a = t._3.get
  ...
  val congressVOForm: Form[CongressVO] = Form(
    ...
    )(CongressVO.apply)(x => Some(x.id, x.title, x.name, x.place, x.country, x.start, x.end, x.international, x.authors)))
}